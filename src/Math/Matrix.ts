...
export namespace Matrix {
  ...
  export function buildPerspective(distanceVector: Vector): Matrix {
    const size = distanceVector.length + 1;
    const w = size - 2;
    const ratio  = 1 / distanceVector[w];
    const matrix = buildIdentity(size);
  ...
  export function buildMinor(matrixA: Matrix, row: number, column: number): Matrix {
    const matrix = [];
    const size = Math.sqrt(matrixA.length) | 0;

    for (let y = 0, yM = 0; y < size; ++y) {
      if (y !== row) {
        for (let x = 0, xM = 0; x < size; ++x) {
          if (x !== column) {
