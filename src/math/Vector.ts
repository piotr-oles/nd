...
export namespace Vector {
  ...
  export function cosinus(vectorA: Vector, vectorB: Vector): number {
    const aDistance = distance(vectorA, vectorB);
    const aDot = dot(vectorA, vectorB);

    return aDistance === 0.0 ? NaN : aDot / aDistance;
  }

  export function sinus(vectorA: Vector, vectorB: Vector): number {
    const aCosinus = cosinus(vectorA, vectorB);

    if (!isNaN(aCosinus)) {
      return Math.sqrt(1 - aCosinus * aCosinus);
    } else {
      return NaN;
    }
  }
